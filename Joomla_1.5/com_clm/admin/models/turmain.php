...
class CLMModelTurMain extends JModel {
	...
	function _getTurniere() {
		...
		foreach ($this->turniere as $key => $value) {
			...
			// Verein
			if ($value->vereinZPS != NULL AND $value->vereinZPS != '0') {
				$query = 'SELECT Vereinname'
							. ' FROM dwz_vereine'
							. ' WHERE ZPS = '.$value->vereinZPS
							;
				$this->_db->setQuery($query);
				$this->turniere[$key]->vereinName = $this->_db->loadResult();
			}
	...
	function _sqlOrder() {
		...
		$arrayOrderAllowed = array('a.name', 'a.typ', 'a.runden', 'a.dg', 'a.teil', 'a.tl', 'c.name', 'a.rnd', 'a.published', 'a.ordering', 'a.id');
		// passt?
