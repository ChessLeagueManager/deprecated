...
function meldelisten ( &$rows, &$lists, &$pageNav, $option )
	{
		...
		?>
		<form action="index.php?option=com_clm&section=meldelisten" method="post" name="adminForm">
...
function setMeldelisteToolbar($row)
	...
	require_once(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_clm'.DS.'images'.DS.'admin_menue_images.php');
		...
		JToolBarHelper::custom( 'save_meldeliste', 'save.png', 'save_f2.png', 'SAVE',false );
		JToolBarHelper::custom( 'apply_meldeliste', 'apply.png', 'apply_f2.png', 'APPLY',false );
...
<?php 	for ($i=0; $i<$liga[0]->stamm; $i++){ ?>
	...
	<!---		 <option value="<?php //echo $row_spl[$x]->id.'-'.$row_spl[$x]->zps; ?>" <?php //if (((int)$row_spl[$x]->id) == ((int)$row_sel[$i]->mgl_nr) AND ($row_spl[$x]->zps == $row_sel[$i]->zps)) { ?> selected="selected" <?php //} ?>><?php //echo $row_spl[$x]->id.'&nbsp;';if((int)$row_spl[$x]->id < 1000) {echo "&nbsp;&nbsp;";} echo "-&nbsp;&nbsp;".$row_spl[$x]->name; ?></option> ---> 
			 <option value="<?php echo $row_spl[$x]->id.'-'.$row_spl[$x]->zps; ?>" <?php if (((int)$row_spl[$x]->id) == ((int)$row_sel[$i]->mgl_nr) AND ($row_spl[$x]->zps == $row_sel[$i]->zps)) { ?> selected="selected" <?php } ?>><?php echo $row_spl[$x]->name.'&nbsp;-&nbsp;&nbsp;'.$row_spl[$x]->id; ?></option> 
			<?php }	?>
		  </select>
		</td>
		<td align="center">
		  <input type="checkbox" name="check<?php echo $i+1; ?>" value="1" <?php if($row_sel[$i]->gesperrt =="1") { echo 'checked="checked"'; }?>>
		</td>
	</tr>
<?php } ?> 
		</table>
 ...
 <fieldset class="adminform">
   <legend><?php echo JText::_( 'MELDELISTE_DETAILS' ); ?></legend>
	...
	<tr>
		...
		<td class="key" nowrap="nowrap"><?php if (!$abgabe[0]->name) {echo "---";} 
			else { echo $abgabe[0]->name; } ?>
		</td>
	</tr>
		<td class="key" nowrap="nowrap"><?php echo JText::_( 'DATE' ).' : '; ?></td>
		<td class="key" nowrap="nowrap"><?php if (!$abgabe[0]->datum OR $abgabe[0]->datum=="0000-00-00 00:00:00") {echo  "---";} 
	...
	<tr>
		...
		<td class="key" nowrap="nowrap"><?php if (!$abgabe[0]->editor ) {echo "---";} 
			else { echo $abgabe[0]->name; } ?>
		</td>
	</tr>
	<tr>
		<td class="key" nowrap="nowrap"><?php echo JText::_( 'DATE' ).' : '; ?></td>
		<td class="key" nowrap="nowrap"><?php if (!$abgabe[0]->edit_datum OR $abgabe[0]->edit_datum =="0000-00-00 00:00:00") {echo  "---";} 
