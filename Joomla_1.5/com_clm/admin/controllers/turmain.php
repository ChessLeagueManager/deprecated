...
class CLMControllerTurMain extends JController {
	...
	*/
	function _copyDo() {
		...
		if (!$tournament->checkAccess()) {
			JError::raiseWarning( 500, CLMText::errorText('COPY', 'NOACCESS') );
			return FALSE;
		}
	...
	*/
	function _createRoundsDo() {
		...
		if ($row->typ == 3) {
			...
			// alle Runden durchgehen - werden umgekehrt benannt -> 3 = VF, 2 = HF, 1 = Finale
			for ($r=$runden; $r>=1; $r--) {
		...
		///////////////////////
			...
			for ($i = 1; $i < 1+$dg; $i++) {
				...
				for ($p = 2; $p < $n; $p++ ) {
					...
					$sqlValuesStrings[] = "('$sid','$turnierid','$roundCount','1','$i','$dgh','$heim','$heim','$gast')";
					$sqlValuesStrings[] = "('$sid','$turnierid','$roundCount','1','$i','$dgg','$gast','$gast','$heim')";
					...
					if ( $gerade == 0 ) {
						for ($z = 2; $z < ($y+1); $z++) {
							$heim = $z+$y-1;
							$gast = $p-$z-$y+2;
							$sqlValuesStrings[] = "('$sid','$turnierid','$roundCount','$z','$i','$dgh','$heim','$heim','$gast')";
							$sqlValuesStrings[] = "('$sid','$turnierid','$roundCount','$z','$i','$dgg','$gast','$gast','$heim')";
						}
						for ($z = ($y+1); $z < (($n/2)+1); $z++) {
							$heim = $z+$y-1;
							$gast = $n+$p-$z-$y+1;
	...
	*/
	function _deleteDo() {
		...
		// access?
		$tournament = new CLMTournament($turnierid, TRUE);
		if (!$tournament->checkAccess(TRUE, FALSE, FALSE)) {
			JError::raiseWarning( 500, JText::_('TOURNAMENT_NO_ACCESS') );
			return FALSE;
		}
