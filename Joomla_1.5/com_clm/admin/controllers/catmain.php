	...
	function publish() {
		...
		if (empty( $cid )) { 
			...
			foreach ($cid as $key => $value) {
				...
				if (CLM_usertype != 'admin' AND CLM_usertype != 'tl') {
					...
					if ($row->published != $publish) {
						// Log
						$clmLog = new CLMLog();
						$clmLog->aktion = JText::_('CATEGORY')." ".$row->name.": ".$task;
			...
			if ( !empty($cid) ) { 
				...
				$app =& JFactory::getApplication();
				if ($publish) {
					$app->enqueueMessage( CLMText::sgpl(count($cid), JText::_('CATEGORY'), JText::_('CATEGORIES'))." ".JText::_('CLM_PUBLISHED') );
				} else {
					$app->enqueueMessage( CLMText::sgpl(count($cid), JText::_('CATEGORY'), JText::_('CATEGORIES'))." ".JText::_('CLM_UNPUBLISHED') );
				}
