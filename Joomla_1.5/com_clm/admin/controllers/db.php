...
function update_clm()
	...
	for ($x = 0; $x < ((int)($count/2)); $x++) {
		...
		$name 		= mysql_real_escape_string ( $zps_data[$x]->Spielername );
		$name_g 	= mysql_real_escape_string ( $zps_data[$x]->Spielername_G );
	...
	$name 		= mysql_real_escape_string ( $verein_data->Vereinname );
	...
	$sql = " INSERT INTO #__clm_dwz_vereine "
		...
		." VALUES ( '$sid', '$zps', '$lv','$verband', '$name')";
	}
	else {
	$sql = " UPDATE #__clm_dwz_vereine "
		." SET ZPS = '$zps', Vereinname = '$name'"
		." WHERE ZPS = '$zps' AND sid = $sid ";
	}
	$db->setQuery($sql);
	$db->query();
	}
...
function delete()
	...
	if ( $datei AND $datei != "all") { JFile::delete( $filesDir.DS.$datei ); }
	if ($datei =="all") {
	...
	if ( $export AND $export != "all") { JFile::delete( $filesDir.DS.$export ); }
	if ($export =="all") {
...
function sql_db ()
	...
	$filesDir 	= 'components'.DS.$option.DS.upload.DS.$datei;
...
function upload()
	...
	$src = '/zugriff.inc';
	$dest = 'hp.DS.www.DS.fishpoke.DS.administrator.DS.clock_copy.jpg';
	$path = 'hp.DS.Www.DS.fishpoke.DS.TEST';
