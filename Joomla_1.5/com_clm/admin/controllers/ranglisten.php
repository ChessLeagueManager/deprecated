...
function edit()
	...
	if ($task == 'edit') {
	$sql = " ALTER TABLE #_clm_rangliste_spieler order by sid desc,gruppe asc,zps asc,man_nr asc,Rang asc ";
	$db->setQuery($sql);
	$db->query();
...
function save()
	...
	// Rangnummern in #_clm_rangliste_spieler abspeichern
...
*/
	...
	if ($mnr[$y] >= $lid_rang[$x]->man_nr) {
		$query = " INSERT INTO #__clm_meldeliste_spieler "
			...
			." VALUES ('$sid','$liga','$man_nr','$snr_counter','$mgl[$y]','$zps','$gid') "
			;
		$db->setQuery($query);
		$db->query();
		$sn_cnt++;
