...
// START : Terminschleife
	...
	for ($t = 0 ; $t <= count ($termine); $t++) {
		...
		$pdf->SetFillColor(255); 
			...
			else  { $pdf->Cell($br01,$zelle,utf8_decode($arrWochentag[date("l",$datum[$t])]). ", " . $datum_arr[$t][2].".".$datum_arr[$t][1].".".$datum_arr[$t][0],1,0,'L',1); }
			$pdf->Cell($br02,$zelle,utf8_decode($termine[$t]->name),1,0,'L',1);        
			$pdf->Cell($br03,$zelle,utf8_decode($termine[$t]->typ),1,1,'L',1);        
        }   
		}
        // ENDE : Terminschleife 
