...
if ($free_date_new[0]->nr_aktion  == 201) $hint_freenew = JText::_('ROUND_FREE_COMMENT').' '.utf8_decode(JText::_('ON_DAY')).' '.JHTML::_('date',  $free_date_new[0]->datum, JText::_('%d. %B %Y')); 
if ($free_date_new[0]->nr_aktion  == 202) $hint_freenew = JText::_('ROUND_FREE_COMMENT_DEL').' '.utf8_decode(JText::_('ON_DAY')).' '.JHTML::_('date',  $free_date_new[0]->datum, JText::_('%d. %B %Y')); 
...
if (!$liga OR $liga[0]->published == "0") {
	...
	$pdf->Cell(10,15,utf8_decode($liga[0]->name." ".$saison[0]->name.', Runde '.$runde),0,1);
...
$pdf->SetFont('Times','',$head_font);
	$pdf->Cell(10,6,' ',0,0);
	$pdf->Cell(170,6,utf8_decode($liga[0]->name)." ".$saison[0]->name,0,1,'C');
$pdf->SetFont('Times','',$head_font-1);
	$pdf->Cell(10,5,' ',0,0);
	$pdf->Cell(170,5,utf8_decode($runden_text).' '.JText::_('ON_DAY').' '.utf8_decode(JHTML::_('date', $liga[$runde-1]->datum, JText::_('%d. %B %Y'))),0,1,'C');
...
// Ergebnis Mannschaft
		...
		if (($hsum == $gsum) AND ($runden_modus == 4 OR $runden_modus == 5)) {
			...
			if (!$cr) $pdf->SetX($xx1); else $pdf->SetX($xx2);
			$pdf->Cell(4,$zelle+1,'','LTB',0);
			$pdf->Cell($breite1*2+5,$zelle+1,utf8_decode($ztext),'TB',0,'C');
			$pdf->Cell(4,$zelle+1,'','RTB',1);
		}}
	else {
		$pdf->Cell(4,$zelle+1,'','LTB',0);
		$pdf->Cell($breite1*2+5,$zelle+1,JText::_('NO_RESULT_YET'),'TB',0,'C');
		$pdf->Cell(4,$zelle+1,'','RTB',1);
		}
		if ($paar[$y]->comment != "") { 
			$ztext = JText::_('PAAR_COMMENT').$paar[$y]->comment; 		
			if (!$cr) $pdf->SetX($xx1); else $pdf->SetX($xx2);
			//$pdf->Cell(4,$zelle+1,'','LTB',0);
			//$pdf->Cell($breite1*2+5,$zelle+1,utf8_decode($ztext),'TB',0,'C');
			//$pdf->Cell(4,$zelle+1,'','RTB',1);
			$pdf->MultiCell($breite,$zelle+1,utf8_decode($ztext),1,'J');
		}
	$z2+=2;
	}
}
...
$diff = $spielfrei[0]->count; 
	...
	$pdf->Cell($breite1+1,$zelle+1,JText::_('TEAM'),'TB',0,'L');
	$pdf->Cell(9,$zelle+1,JText::_('SP'),'TB',0,'C');
	$pdf->Cell(10,$zelle+1,JText::_('MP'),'TB',0,'C'); 
	$tbreite = $breite1 + 26;
	if ( $liga[0]->liga_mt == 0) { 
		$tbreite = $tbreite + 10;
...
for ($x=0; $x< ($liga[0]->teil)-$diff; $x++){
	...
	$pdf->Cell($breite1+1,$zelle,utf8_decode($punkte[$x]->name),0,0,'L');
	$pdf->Cell(9,$zelle,$punkte[$x]->spiele,0,0,'C');
	$pdf->Cell(10,$zelle,$punkte[$x]->mp,0,0,'C');
	if ( $liga[0]->liga_mt == 0) {
...
// Ansetzungen Folgerunde
	...
	if (!$cr) $pdf->SetX($xx1); else $pdf->SetX($xx2);
	$pdf->Cell($breite,6,utf8_decode(JText::_('ROUND_PROGRAM')).' '.utf8_decode($runden_text).' '.JText::_('ON_DAY').' '.utf8_decode(JHTML::_('date', $liga[$runde]->datum, JText::_('%d. %B %Y'))),0,1,'L');
	//$pdf->Cell($breite,6,utf8_decode(JText::_('ROUND_PROGRAM')).' '.utf8_decode(JText::_('ROUND')).' '.utf8_decode($runden_text).' '.JText::_('ON_DAY').' '.utf8_decode(JHTML::_('date', $liga[$runde]->datum, JText::_('%d. %B %Y'))),0,1,'L');
	for ($y=0; $y< ($liga[0]->teil)/2; $y++){ 
