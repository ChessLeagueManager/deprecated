...
if ($spieler[0]->dsb_datum  > 0) $hint_dwzdsb = JText::_('DWZ_DSB_COMMENT_RUN').' '.utf8_decode(JText::_('ON_DAY')).' '.JHTML::_('date',  $spieler[0]->dsb_datum, JText::_('%d. %B %Y')); 
	...
	<?php $hint_dwznew = "";
		  foreach ($dwz_date_new as $dwz_date_liga) {
			if ($dwz_date_liga->lid == $spielerl->liga) {
				if ($dwz_date_liga->nr_aktion  == 101) $hint_dwznew = JText::_('DWZ_COMMENT_RUN').' '.utf8_decode(JText::_('ON_DAY')).' '.JHTML::_('date',  $dwz_date_liga->datum, JText::_('%d. %B %Y ,  %H %M')); 
				if ($dwz_date_liga->nr_aktion  == 102) $hint_dwznew = JText::_('DWZ_COMMENT_DEL').' '.utf8_decode(JText::_('ON_DAY')).' '.JHTML::_('date',  $dwz_date_liga->datum, JText::_('%d. %B %Y ,  %H %M'));  
				break;
		  } }
	...
	<?php $hint_dwznew = "";
		  foreach ($dwz_date_new as $dwz_date_liga) {
			if ($dwz_date_liga->lid == 0) {
				if ($dwz_date_liga->nr_aktion  == 101) $hint_dwznew = JText::_('DWZ_COMMENT_SRUN').' '.utf8_decode(JText::_('ON_DAY')).' '.utf8_decode(JHTML::_('date',  $dwz_date_liga->datum, JText::_('%d. %B %Y ,  %H %M')));  
				if ($dwz_date_liga->nr_aktion  == 102) $hint_dwznew = JText::_('DWZ_COMMENT_DEL').' '.utf8_decode(JText::_('ON_DAY')).' '.utf8_decode(JHTML::_('date',  $dwz_date_liga->datum, JText::_('%d. %B %Y ,  %H %M')));  
				break;
		  } }
		  if ($hint_dwznew == "") $hint_dwznew = JText::_('DWZ_COMMENT_UNCLEAR'); //klkl 
	?>
