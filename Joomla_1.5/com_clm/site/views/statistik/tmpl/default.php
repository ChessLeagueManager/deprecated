...
<div id="pdf">
    <div class="pdf"><a href="index.php?option=com_clm&view=statistik&format=clm_pdf&layout=brettbeste&saison=<?php echo $liga[0]->sid ?>&liga=<?php echo $liga[0]->id ?>&runde=<?php echo $runde_orig ?>&dg=<?php echo $dg ?>"><img src="<?php echo $imageDir; ?>pdf_button.png" alt="PDF"  class="CLMTooltip" title="<?php echo JText::_('LEAGUE_STAT_PDF') ?>"  /></a>
</div></div></div>
<div class="clr"></div>
...
<table cellpadding="0" cellspacing="0" class="statistik">
	<tr>
		...
		<?php for ($xx=1; $xx < 7; $xx++) {   //max. 6 Spalten
			if ($params['btiebr'.$xx] == 1) $hstring = JText::_('LEAGUE_STAT_PLAYERPOINTS');
			elseif ($params['btiebr'.$xx] == 2) $hstring = JText::_('LEAGUE_STAT_PLAYERGAMES');
			elseif ($params['btiebr'.$xx] == 3) $hstring = JText::_('LEAGUE_STAT_PLAYERLEVEL');
			elseif ($params['btiebr'.$xx] == 4) $hstring = JText::_('LEAGUE_STAT_RATING');
			elseif ($params['btiebr'.$xx] == 5) $hstring = JText::_('LEAGUE_STAT_PERCENT');
			elseif ($params['btiebr'.$xx] == 6) $hstring = JText::_('LEAGUE_STAT_POINTS_K');
			elseif ($params['btiebr'.$xx] == 7) $hstring = JText::_('LEAGUE_STAT_GAMES_K');
			elseif ($params['btiebr'.$xx] == 8) $hstring = JText::_('LEAGUE_STAT_PERCENT_K');
			elseif ($params['btiebr'.$xx] == 9) $hstring = JText::_('LEAGUE_STAT_BNUMBERS');
...
if ($count < 10) { $a = $count; }
	...
	<tr class="<?php echo $zeilenr; ?>">
		...
		<?php for ($xx=1; $xx < 7; $xx++) {   //max. 6 Spalten
			...
			elseif ($params['btiebr'.$xx] == 4) if (($bestenliste[$x]->Punkte == $bestenliste[$x]->Partien) AND ($bestenliste[$x]->Leistung > 0)) { $hstring = (($bestenliste[$x]->Niveau)+667).' &sup2;'; $ex = 1; } else { $hstring = $bestenliste[$x]->Leistung;}
			elseif ($params['btiebr'.$xx] == 5) $hstring = round($bestenliste[$x]->gprozent,1);
			elseif ($params['btiebr'.$xx] == 6) { $hstring = $bestenliste[$x]->epunkte; $ey = 1; }
			elseif ($params['btiebr'.$xx] == 7) { $hstring = $bestenliste[$x]->epartien; $ey = 1; }
			elseif ($params['btiebr'.$xx] == 8) { $hstring = round($bestenliste[$x]->eprozent,1); $ey = 1; }
...
<table cellpadding="0" cellspacing="0" class="statistik">
	<tr>
		...
		<th><?php echo JText::_('LEAGUE_STAT_CLUB') ?></th>
		<?php for ($xx=1; $xx < 7; $xx++) {   //max. 6 Spalten
			if ($params['btiebr'.$xx] == 1) $hstring = JText::_('DWZ_POINTS');
			elseif ($params['btiebr'.$xx] == 2) $hstring = JText::_('DWZ_GAMES');
			elseif ($params['btiebr'.$xx] == 3) $hstring = JText::_('DWZ_LEVEL');
			elseif ($params['btiebr'.$xx] == 4) $hstring = JText::_('LEAGUE_STAT_RATING');
			elseif ($params['btiebr'.$xx] == 5) $hstring = JText::_('LEAGUE_STAT_PERCENT');
			elseif ($params['btiebr'.$xx] == 6) $hstring = JText::_('LEAGUE_STAT_POINTS_K');
			elseif ($params['btiebr'.$xx] == 7) $hstring = JText::_('LEAGUE_STAT_GAMES_K');
...
<?php $xb = 1;
	...
	<tr class="<?php echo $zeilenr; ?>">
		...
		<?php for ($xx=1; $xx < 7; $xx++) {   //max. 6 Spalten
			if ($params['btiebr'.$xx] == 1) $hstring = $spielerbrett->gpunkte;
			elseif ($params['btiebr'.$xx] == 2) $hstring = $spielerbrett->gpartien;
			elseif ($params['btiebr'.$xx] == 3) $hstring = $spielerbrett->Niveau;
			elseif ($params['btiebr'.$xx] == 4) if (($spielerbrett->Punkte == $spielerbrett->Partien) AND ($spielerbrett->Leistung > 0)) { $hstring = (($spielerbrett->Niveau)+667).' &sup2;'; $ex = 1; } else { $hstring = $spielerbrett->Leistung;}
			elseif ($params['btiebr'.$xx] == 5) $hstring = round($spielerbrett->gprozent,1);
			elseif ($params['btiebr'.$xx] == 6) $hstring = $spielerbrett->epunkte;
			elseif ($params['btiebr'.$xx] == 7) $hstring = $spielerbrett->epartien;
			elseif ($params['btiebr'.$xx] == 8) $hstring = round($spielerbrett->eprozent,1);
			elseif ($params['btiebr'.$xx] == 9) $hstring = $spielerbrett->ebrett;
			if ($params['btiebr'.$xx] > 0) { ?>
		<td align="center"><?php echo $hstring ?></td>
			<?php } } ?>
	</tr>
...
<?php for ($x=0; $x < count($stat_kampflos); $x++) {
	...
	<tr class="<?php echo $zeilenr; ?>">
		<td align="center"><?php echo $x+1; ?></td>
		<td align="center"><a href="index.php?option=com_clm&view=mannschaft&saison=<?php echo $sid; ?>&liga=<?php echo $lid; ?>&tlnr=<?php echo $stat_kampflos[$x+1]->tln_nr; ?><?php if ($itemid <>'') { echo "&Itemid=".$itemid; } ?>"><?php echo $stat_kampflos[$x+1]->name; ?></a></td>
		<td align="center"><?php echo $stat_kampflos[$x+1]->kg_sum; ?></td>
		<td align="center"><?php echo $stat_kampflos[$x+1]->kv_sum; ?></td>
	</tr>
<?php } ?>
</table>
<?php } ?>
