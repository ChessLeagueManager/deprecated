...
if ($termin[$term]->datum AND $termin[$term]->nr == ($x+1)) { $datum = ', '.JHTML::_('date',  $termin[$term]->datum, JText::_('%d. %B %Y')); $term++;}
...
for ($y=0; $y< ($liga[0]->teil)/2; $y++){
	...
	if ($dwzgespielt[$z2]->dwz AND $dwzgespielt[$z2]->runde == ($x+1) AND $dwzgespielt[$z2]->paar == ($y+1) AND $dwzgespielt[$z2]->dg == 1 AND $paar[$z]->hmnr !=0 AND $paar[$z]->gmnr != 0)
		{ $pdf->Cell(12-$breite,$zelle,round($dwzgespielt[$z2]->dwz),1,0,'C'); }
		else { $pdf->Cell(12-$breite,$zelle,round($dwz[($paar[$z]->htln)]),1,0,'C');}
...
if ( $summe[$sum_paar]->runde == ($x+1) AND $summe[$sum_paar]->paarung == ($y+1)) {
	...
	if ($dwzgespielt[$z2]->dwz AND $dwzgespielt[$z2]->runde == ($x+1) AND $dwzgespielt[$z2]->paar == ($y+1) AND $dwzgespielt[$z2]->dg == 1 AND $paar[$z]->hmnr !=0 AND $paar[$z]->gmnr != 0)
		...
		else { $pdf->Cell(12-$breite,$zelle,round($dwz[($paar[$z]->gtln)]),1,0,'C'); }
if ($remis_com == 1) { 
...
if ($termin[$term]->datum AND $termin[$term]->nr == ($x+1+$liga[0]->runden)) { $datum = ', '.JHTML::_('date',  $termin[$term]->datum, JText::_('%d. %B %Y')); $term++;}
...
for ($y=0; $y< ($liga[0]->teil)/2; $y++){
	...
	if ($dwzgespielt[$z2]->dwz AND $dwzgespielt[$z2]->runde == ($x+1) AND $dwzgespielt[$z2]->paar == ($y+1) AND $dwzgespielt[$z2]->dg == 2 AND $paar[$z]->hmnr !=0 AND $paar[$z]->gmnr != 0)
		{ $pdf->Cell(12-$breite,$zelle,round($dwzgespielt[$z2]->dwz),1,0,'C'); }
		else { $pdf->Cell(12-$breite,$zelle,round($dwz[($paar[$z]->htln)]),1,0,'C');}
// Wenn Paarung existiert dann Ergebnis-Summen anzeigen
while ( $summe[$sum_paar]->runde < ($x+1) ) $sum_paar++;
