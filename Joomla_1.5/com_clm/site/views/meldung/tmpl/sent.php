...
if (!$id[0]->gemeldet) {
	...
	$query = " SELECT a.stamm, a.sieg, a.remis, a.nieder, a.antritt, a.runden_modus, "
...
// Mailbody HTML Spielbericht
	$body_html =	'
		...
		<tr>
			<td bgcolor="#F2F2F2" style="border-bottom: solid 1px #000000; border-top: solid 1px #000000; padding: 3px;" colspan="6"><div align="center" style="font-size: 12px;"><strong>Online Spielbericht vom ' .JHTML::_('date', date("Y-m-d"), JText::_('%d. %B %Y')). '</strong></div></td>
		</tr>
		<tr>
			<td width="120">&nbsp;</td>
			<td>&nbsp;</td>
		...
		<tr>
			...
			<td style="border-bottom: solid 1px #999999;">' .JHTML::_('date',  $rundeterm[0]->datum, JText::_('%d. %B %Y')). '&nbsp;</td>
		</tr>
		<tr>
...
// BCC
	...
	// Mailbody HTML ML
	$body_html_mf = '<br>'
		.JText::_('RESULT_DATA_BODY1')." ".$begegnung[0]->name." - ".$begegnung[1]->name
		.JText::_('RESULT_DATA_BODY2_1').$fromname.JText::_('RESULT_DATA_BODY2_2')
		.' siehe unten oder '
		.'<a href="http://'.$pfad.'/index.php?option=com_clm&view=runde&saison='.$sid.'&liga='.$lid.'&runde='.$rnd.'&dg='.$dg.'">hier</a>'
		.'<br>'.'<a href="mailto:'.$sl_bcc.'">'.JText::_('RESULT_DATA_BODY3').'</a>'
		.'<br>'.JText::_('RESULT_DATA_BODY4')
		.'<a href="http://'.$pfad.'/index.php?option=com_clm&view=rangliste&saison='.$sid.'&liga='.$lid.'"> Liga</a>'
		.'<br>    '.JText::_('RESULT_DATA_BODY5').'<br><br>'
	;
...
if ($empfang[0]->email AND $empfang[1]->email ) {
	...
	if ($bcc_mail != "") {
		...
		$body_html_ad = '<br>'
			...
			.'<br>'.JText::_('RESULT_ADMIN_COPY3_1').$fromname.JText::_('RESULT_ADMIN_COPY3_2')
			.'<br>'.'<br>'
		;
