...
if ($clmuser[0]->zps != $paar[0]->hzps AND $clmuser[0]->zps != $paar[0]->gzps AND $clmuser[0]->zps != $paar[0]->hsgzps AND $clmuser[0]->zps != $paar[0]->gsgzps) {
	$link = "index.php?option=com_clm&view=info";
...
else { echo ', am '.JHTML::_('date', $liga[0]->datum, JText::_('%d.%m.%Y')); }?>
</h4>
	...
	<tr>
		...
		<td>
		  <select size="1" name="<?php echo "heim".($i+1); ?>" id="<?php echo "heim".($i+1); ?>">
			...
			if ($liga[0]->rang !="0") {?>
			 <option value="<?php echo $heim[$x]->mgl_nr.';'.$heim[$x]->name.';'.$heim[$x]->zps; ?>"><?php echo $heim[$x]->rmnr.' - '.$heim[$x]->rang.' &nbsp;&nbsp;';if($heim[$x]->rang < 1000) { echo "&nbsp;&nbsp;&nbsp;&nbsp;";};if($heim[$x]->rang < 10) { echo "&nbsp;&nbsp;";}; echo $heim[$x]->name; ?></option> 
			<?php }
			else { ?>
			 <option value="<?php echo $heim[$x]->mgl_nr.';'.$heim[$x]->name.';'.$heim[$x]->zps; ?>"><?php echo $heim[$x]->snr;if($heim[$x]->snr < 10) {echo "&nbsp;&nbsp";} echo ' - '.$heim[$x]->name; ?></option> 
			<?php }} ?>
			<option value="<?php echo '99999'.';'.JText::_('RESULTS_DETAILS_NOT_NOMINATED').';'.'ZZZZZ'; ?>"<?php if ($heim[$i]->hzps =="ZZZZZ"){ ?> selected="selected"<?php } ?>>&nbsp;&nbsp;&nbsp;---&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo JText::_('RESULTS_DETAILS_NOT_NOMINATED'); ?></option>
		  </select>
		</td>
<td> - </td>
		<td>
		  <select size="1" name="<?php echo "gast".($i+1); ?>" id="<?php echo "gast".($i+1); ?>">
			...
			if ($liga[0]->rang !="0") {?>
			 <option value="<?php echo $gast[$x]->mgl_nr.';'.$gast[$x]->name.';'.$gast[$x]->zps; ?>"><?php echo $gast[$x]->rmnr.' - '.$gast[$x]->rang.' &nbsp;&nbsp;';if($gast[$x]->rang < 1000) { echo "&nbsp;&nbsp;&nbsp;&nbsp;";};if($gast[$x]->rang < 10) { echo "&nbsp;&nbsp;";}; echo $gast[$x]->name; ?></option> 
			<?php }
			else { ?>
			 <option value="<?php echo $gast[$x]->mgl_nr.';'.$gast[$x]->name.';'.$gast[$x]->zps; ?>"><?php echo $gast[$x]->snr;if($gast[$x]->snr <10) {echo "&nbsp;&nbsp;";} echo ' - '.$gast[$x]->name; ?></option> 
			<?php }} ?>
			<option value="<?php echo '99999'.';'.JText::_('RESULTS_DETAILS_NOT_NOMINATED').';'.'ZZZZZ'; ?>"<?php if ($gast[$i]->hzps =="ZZZZZ"){ ?> selected="selected"<?php } ?>>&nbsp;&nbsp;&nbsp;---&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo JText::_('RESULTS_DETAILS_NOT_NOMINATED'); ?></option>
		  </select>
		</td>
		...
		<td>
		  <select size="1" name="<?php echo "ergebnis".($i+1); ?>" id="<?php echo "ergebnis".($i+1); ?>">
			...
			<?php for ($x=0; $x < 9; $x++) { ?>
			 <option value="<?php echo $ergebnis[$x]->eid.';'.$erg_text[$x]->erg_text; ?>"><?php echo $erg_text[$x]->erg_text ; ?></option> 
			<?php } ?>
		  </select>
		</td>
	</tr>
...
<input type="submit" value=" <?php echo JText::_('RESULT_DATA_BUTTON_NEXT') ?> ">
		...
		<input type="hidden" name="datum" value="<?php echo JHTML::_('date', $liga[0]->datum, JText::_('%d.%m.%Y')); ?>" />
