...
function dwz($sid,$lid)
	...
	$dwz	= 0; //Ligaauswertung!
	$Turnier= $sid;
	$Saison	= $sid;
	// Wenn Liga gesetzt dann Einzelauswertung jeder Liga, wenn "0" dann Auswertung aller Partien dieser Saison !
	if ($dwz =="0") { $Liga	= $lid; }
		else { $Liga = 0;}
	...
	$query	= "INSERT INTO #__clm_log "
		...
		." VALUES ('$aktion','$jid_aktion','$Saison','$lid','$cids','$now') "
		;
	$db->setQuery($query);
	$db->query();
	}
...
$Spalten_DWZListe=array('ZPS'=>'ZPS','Mgl'=>'Mgl_Nr','Jahr'=>'Geburtsjahr','R0'=>'DWZ','I0'=>'DWZ_Index');
...
$query='SELECT '.
       ...
       ' FROM '.mysql_real_escape_string($Tabelle_Einzelergebnisse).
...
/*
  if ($Egb=="1" ) $Partie='1';
  ElseIf ($Egb=="0") $Partie='0';
  ElseIf ($Egb=="2") $Partie='5';
  ElseIf ($Egb=="5") $Partie='+';
  ElseIf ($Egb=="4") $Partie='-';
  ElseIf ($Egb=="7") $Partie='*';
  Else $Partie='?';
...
/*********************************************************************
  Falls Bereich gesetzt ist, kann man damit das ausgegebene Feld einschr�nken.
  $Bereich='7' findet alle Spieler der DWZ-Datenbank, die im NSV gemeldet sind
 *********************************************************************/

if (!isSet($Bereich)) $Bereich='';
Else if (!strrpos('123456789ABCDEFGHLM', substr($Bereich ,0,1))) $Bereich='';
Else $Bereich=" WHERE 1=(ZPS REGEXP '^$Bereich')";
...
$query='SELECT '.
       ...
       $Spalten_DWZListe['R0'].','.
       $Spalten_DWZListe['I0'].' ' .
       'FROM '.mysql_real_escape_string($Tabelle_SpielerDWZ).$Bereich.
       ' ORDER BY ZPS,Mgl_Nr';
$result=&CLMModelMeldung::Abfrage($query);
if(!$result) die (mysql_error().'<br>');
//unset($Bereich);
/*********************************************************************
  Dieser verschachtelte if-Block stellt die Abfrage von 2 Typen dar.
 *********************************************************************/
if ((!is_string($result[0]['ZPS'])) and (!is_integer($result[0]['Mgl_Nr'])))
 {
  foreach ($result as &$row)
  {
   settype($row['ZPS'],'string');
   settype($row['Mgl_Nr'],'integer');
  }
 }
ElseIf (!is_string($result[0]['ZPS']))
 {
  foreach ($result as &$row)
  {
   settype($row['ZPS'],'string');
...
ElseIf (!is_integer($result[0]['Mgl_Nr']))
 {
  foreach ($result as &$row)
  {
   settype($row['Mgl_Nr'],'integer');
...
for ($i=0;$i<sizeof($TempArrAuswertung);$i++)
 {
  for ($j=sizeof($TempArrAuswertung[$i])-1;0<=$j;$j--)
   {
    $k=0;
    while (($k<(sizeof($result)-1))
           and( ($TempArrAuswertung[$i][$j][0]!=$result[$k]['ZPS'])
           or ($TempArrAuswertung[$i][$j][1]!=$result[$k]['Mgl_Nr'])))
     {
      $k++;
     }
    if (($TempArrAuswertung[$i][$j][0]==$result[$k]['ZPS']) and ($TempArrAuswertung[$i][$j][1]==$result[$k]['Mgl_Nr']))
     {
      $TempArrAuswertung[$i][$j]['DWZ']=array($result[$k]['DWZ'],$result[$k]['DWZ_Index'],$result[$k]['Geburtsjahr']);
     }
    Else
...
foreach($TempArrAuswertung as $EinSpieler)
{
 ...
 $ArrUebergabeDWZ[]=array(array('zps'=>$EinSpieler[0][0],'mgl_nr'=>$EinSpieler[0][1]),
    ...
    if (!$result)   // Datensatz nicht vorhanden.
     {
      ...
      Else
       {
        If (Entwanzen)
        echo 'Insert von '.$EinSpieler[0]['zps'].'-'.$EinSpieler[0]['mgl_nr'].' gescheitert!';//,E_USER_NOTICE);
       }
     }   
