...
class CLMModelMannschaft extends JModel
{
	...
	$tln = JRequest::getInt('tlnr');
		...
		$query = "SELECT a.zps,a.sg_zps,u.name as mf_name,u.email as email, "
			." u.tel_mobil,u.tel_fest, l.durchgang as dg, l.rang as lrang,"
	...
	if ($rang > 0) {
		$query = " SELECT m.tln_nr as tln_nr,a.snr,a.dwz,a.mgl_nr,a.zps, d.Spielername as name,d.DWZ as dwz "
		...
		$query = " SELECT a.dwz,a.mgl_nr,a.zps, d.Spielername as name,d.DWZ as dwz "
			." FROM #__clm_meldeliste_spieler as a "
			." LEFT JOIN #__clm_dwz_spieler as d on d.zps = a.zps AND d.mgl_nr = a.mgl_nr AND d.sid = a.sid"
			." WHERE a.sid = ".$sid
			." AND (( a.zps = '$zps' AND a.mnr = $mnr) OR ( a.zps='$sgzps' AND a.mnr = $mnr )) "
